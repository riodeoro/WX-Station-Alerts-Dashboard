...
fetch_data_from_db <- function(conn, table, num_entries = NULL, selected_date = NULL, date_range = NULL, fire_centre) {
  ...
  check_columns_query <- paste0(
    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS ",
    "WHERE TABLE_NAME = '", gsub("dbo.", "", table), 
    "' AND COLUMN_NAME IN ('", paste(additional_columns, collapse = "','"), "')"
  )
  
  existing_columns <- sqlQuery(conn, check_columns_query)$COLUMN_NAME
